...
<script>
    $(function(){
        ...
        function getAllGood(){
            $.ajax({
                url:"/GET/goodsSupplement/1/"+${sessionScope.shopperId},
                type:"get",
                dataType:"json",
                success:function (goodPage){
                    ...
                    $.each(goodPage,function (i,item) {
                        goods.push('<tr>' +
                            ...
                            '<input class="content-table-delete" name="'+item.goodId+'"  type="button" value="补货"/></div>' +
                            '<input class="content-table-edit" name="'+item.goodId+'"  type="button" value="查看详情"/></div></td>'+
                            '</tr>')
                    })
