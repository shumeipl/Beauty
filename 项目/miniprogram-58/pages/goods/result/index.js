...
Page({
  ...
  async init(reset = true) {
    ...
    const params = this.generalQueryData();
    ...
    try {
      const result = await this.queryGoodInfo();
      const code = 'Success';
      const data = result;
  ...
  queryGoodInfo(){
  wx.request({
    url: 'http://localhost:8081/GET/GoodByName/'+this.data.keywords,
    method:"GET",
    header:{
  ...
  handleSubmit() {
    this.queryGoodInfo(this.keyword);
    this.setData(
      {
        goodsList: [],
        loadMoreStatus: 0,
  ...
  gotoGoodsDetail(e) {
    ...
    console.log(e.detail.goods);
    wx.navigateTo({
      url: '/pages/goods/details/index?goodId='+e.detail.goods.goodId+'',
    });
  },
